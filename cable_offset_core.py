    ...
    for di,d in enumerate(designs or []):
        ...
        for si,s in enumerate(d.get("segments",[]) or []):
            ...
            for ni,p in enumerate(nodes):
                ...
                out.setdefault((round(p.x(),7),round(p.y(),7)),[]).append((di,si,ni,len(nodes),_special(item),_kind(item)))
    return out

def _plan(designs,dc,edge_layer,spacing):
    edge_crs=edge_layer.crs();work=_metric_crs(edge_layer,dc);pending,routes=_route._collect_uses(designs,edge_crs,work)
    ...
    occ=_occupancy(dc,designs,work);idx,geoms=_base._build_existing_index(occ,work);cache={};used={};slots={};uses={d:[] for d in ordered}
    for u in pending:uses.setdefault(u.design_index,[]).append(u)
    owners={};rank={d:i for i,d in enumerate(ordered)}
    ...
    def reserve(edge):
        if edge not in cache:
            a,b=_base._edge_points(edge);a=_tp(a,edge_crs,work);b=_tp(b,edge_crs,work);cache[edge]=_base._existing_slot_occupancy(QgsGeometry.fromPolylineXY([a,b]),spacing,idx,geoms)
    ...
    for di in ordered:
        ...
        for u in sorted(uses.get(di,[]),key=lambda x:(x.segment_index,x.edge_index)):
            ...
            edge=u.edge_key;ue=used.setdefault(edge,set(reserve(edge)));a,b=_base._edge_points(edge);a=_tp(a,edge_crs,work);b=_tp(b,edge_crs,work);ka=(round(a.x(),7),round(a.y(),7));kb=(round(b.x(),7),round(b.y(),7))
            candidates=[]
            if prev is None and 0 not in ue and owners.get(ka,di)==di and owners.get(kb,di)==di:candidates.append(0)
            if prev is not None:
                candidates.append(int(prev));sgn=1 if prev>0 else -1 if prev<0 else (1 if u.side_hint>=0 else -1);candidates += [sgn*m for m in range(max(1,abs(int(prev))+1),101)]+[-sgn*m for m in range(1,101)]
            else:
                sgn=1 if u.side_hint>=0 else -1;candidates += [sgn*m for m in range(1,101)]+[-sgn*m for m in range(1,101)]
            chosen=next((int(c) for c in candidates if int(c) not in ue),None)
            if chosen is None:chosen=(max([abs(int(x)) for x in ue]+[0])+1)*(1 if u.side_hint>=0 else -1)
            slots[(di,u.segment_index,u.edge_index)]=chosen;u.slot=chosen;ue.add(chosen);prev=chosen;prevseg=u.segment_index
...
def _geometry(seg,slots,spacing,work,edge_crs,control):
    ...
    for i,slot in enumerate(sl):
        ...
        if i==0:add(a) if slot==0 or not special else add(_geom._takeoff_entry(a,b,slot,spacing,control))
        if i>0:
            prev=sl[i-1]
            if prev==slot:add(a) if slot==0 else add(_geom._same_lane_join(a,nodes[i-1],nodes[i],a,b,slot,spacing))
            elif slot==0:add(a)
...
def apply_offset_layout(designs,distribution_layer,edge_layer,spacing=DEFAULT_SPACING_M,control_distance_m=DEFAULT_CONTROL_M):
    spacing=max(.01,float(spacing));control=max(.01,float(control_distance_m));_set_flags(designs);edge_crs,work,ordered,routes,slot_map=_plan(designs,distribution_layer,edge_layer,spacing);changed=set();extra=0.
    for di,d in enumerate(designs or []):
        if d.get("written") and not d.get("needs_resync"):continue
        new=[];total=0.;diff=False
        for si,s in enumerate(d.get("segments",[]) or []):
            ss={ei:slot_map.get((di,si,ei),0) for ei,x in enumerate(s.get("edge_sequence",[]) or []) if _base._canonical_edge(x) is not None};nw=_geometry(s,ss,spacing,work,edge_crs,control);old=[_tp(QgsPointXY(float(p[0]),float(p[1])),edge_crs,work) for p in s.get("points",[]) if len(p)>=2]
            if len(nw)>=2:
                g=QgsGeometry.fromPolylineXY(nw);og=QgsGeometry.fromPolylineXY(old) if len(old)>=2 else QgsGeometry();diff=diff or len(nw)!=len(old) or any(hypot(a.x()-b.x(),a.y()-b.y())>1e-7 for a,b in zip(nw,old));out=[_tp(p,work,edge_crs) for p in nw];cp=dict(s);cp["points"]=[[float(p.x()),float(p.y())] for p in out];cp["distance"]=round(g.length(),3);cp["layout_spacing"]=round(spacing,3);new.append(cp);total+=g.length();extra+=max(0.,g.length()-(og.length() if not og.isEmpty() else g.length()))
            else:new.append(s)
    ...
    _validate(designs,edge_crs,work);_log(f"[OffsetCore] links={len(ordered)}; changed={len(changed)}; spacing={spacing:.3f}m; control={control:.3f}m; ordinary_corner_control=NOT_USED")
