...
ODN Project scope when no project-scoped state exists. For old written Links
without stored DC feature ids, recover ids only when every stored segment has
exactly one matching Distribution Cable geometry; ambiguous matches are
    ...
    def load_design_for_edit(self, index):
        ...
        if 0 <= index < len(self._designs) and self._designs[index].get("written"):
            if self._recover_written_fids(index) is None:
                QtWidgets = __import__("qgis.PyQt.QtWidgets", fromlist=["QMessageBox"]).QMessageBox
                QtWidgets.warning(
                    self,
                    "修改 Link",
