...
def _stable_link_id(design, index=None):
    ...
    if basis:
        link_id = "LNK_" + hashlib.sha1(basis.encode("utf-8")).hexdigest()[:20].upper()
    else:
        link_id = "LNK_" + uuid.uuid4().hex.upper()
...
def _v12_add_fat(self, info):
    ...
    self._persist_state(); self._refresh_ui()
    if self._tool:
...
def _v12_save_current_link(self):
    ...
    try:
        ...
        for index, fids in sorted(sources.items(), reverse=True):
            ...
            rebuilt["written"] = False; rebuilt["needs_resync"] = True
            self._designs[index] = rebuilt
        result = _original_save_current_link(self)
        if not result:
            self._designs = backup; self._pending_reassignments = pending
            self._persist_state(); self._refresh_ui(); return False
        _prepare_design_identities(self._designs); self._pending_reassignments = {}; self._persist_state(); return True
    except Exception as exc:
        self._designs = backup; self._pending_reassignments = pending
        self.status.setText(f"状态：保存失败：{exc}"); self._persist_state(); self._refresh_ui()
        QtWidgets.QMessageBox.warning(self, "保存规划", str(exc)); return False
...
def _sync_dc_changes_into_designs(self, layer):
    ...
    for index, design in enumerate(self._designs or []):
        link_id = _stable_link_id(design, index); segments = design.get("segments", []) or []
        new_segments = copy.deepcopy(segments); changed = False
        for seg_index, segment in enumerate(segments):
            ...
            if old_geom is None or not _same_geometry(old_geom, new_geom):
                new_segments[seg_index]["points"] = points; changed = True
        if changed:
            design["segments"] = new_segments; design["written"] = True; design["needs_resync"] = False; changed_links.append(index)
    if changed_links:
        self._persist_state()
...
def _write_design_to_dc(self, layer, design_index, design):
    ...
    for key, items in list(existing.items()):
        ...
        if key not in wanted:
            ...
            continue
        target_geom = wanted[key][1]; kept = False
...
def _v12_write_planned_links(self):
    ...
    if layer is None:
        QtWidgets.QMessageBox.warning(self, "写入图层", "当前项目没有绑定 Distribution Cable 图层。"); return False
    if not self._designs:
        QtWidgets.QMessageBox.information(self, "写入图层", "当前没有已保存的 Link 规划。"); return True
    if not layer.isEditable() and not layer.startEditing():
        QtWidgets.QMessageBox.warning(self, "写入图层", f"无法进入 Distribution Cable 编辑状态：{layer.name()}"); return False
    try:
        _prepare_design_identities(self._designs); links = segments = 0
        for index, design in enumerate(self._designs):
            if not (design.get("segments") or []): continue
            _write_design_to_dc(self, layer, index, design); design["written"] = True; design["needs_resync"] = False; design.pop("_resync_source", None); links += 1; segments += len(design.get("segments") or [])
        layer.triggerRepaint(); self._persist_state()
    except Exception as exc:
        QtWidgets.QMessageBox.warning(self, "写入图层", f"同步 Distribution Cable 失败：\n{exc}"); return False
    self._refresh_ui(); QtWidgets.QMessageBox.information(self, "写入图层", f"已同步 {links} 条 Link，共 {segments} 个线路段。\n\n已存在的 Link/Segment 按稳定内部 ID 更新；FDT/FAT 名称变化不会改变归属。"); return True
...
def _show_startup_sync_dialog(self, layer):
    ...
    if not _ensure_sync_fields(layer):
        QtWidgets.QMessageBox.warning(self, "Link 数据检查", "Distribution Cable 无法建立内部 Link 标识字段，无法安全判断线路归属。"); return False
    _sync_dc_changes_into_designs(self, layer)
    records = _dc_records(layer); known = {_stable_link_id(d, i) for i, d in enumerate(designs)}
    orphan = [key for key in records if key[0] not in known]
    if not orphan:
        self._persist_state(); return True
