...
class OverlengthPoleDialog(QtWidgets.QDialog):
    """Layer bindings come from the active ODN Project; distance rules are editable."""
    ...
    def __init__(self, iface, parent=None):
        ...
        self.ee = self._distance_edit(65.0)
        self.en = self._distance_edit(65.0)
        self.nn = self._distance_edit(50.0)
        g.addWidget(QtWidgets.QLabel("Existing Pole-Existing Pole"), 0, 0)
        g.addWidget(self.ee, 0, 1)
        g.addWidget(QtWidgets.QLabel("Existing Pole-New Pole"), 1, 0)
        g.addWidget(self.en, 1, 1)
        g.addWidget(QtWidgets.QLabel("New Pole-New Pole"), 2, 0)
        g.addWidget(self.nn, 2, 1)
        lay.addWidget(box)

        tip = QtWidgets.QLabel(
            "图层自动读取当前 ODN Project。\n"
            "处理逻辑保持原有方式：优先利用线路附近已有杆，仍超距时自动新增 New Pole，并自动选中新杆。"
        )
        ...
        ok.clicked.connect(self._start); cancel.clicked.connect(self.reject)
    ...
    def _distance_edit(value):
        ...
        edit.setValidator(QDoubleValidator(0.01, 1000000.0, 2, edit))
        return edit

    def _load_project_layers(self):
        payload = context.require_project(self, "超距增点")
        if not payload:
            self._ok_enabled(False); return
    ...
    def _start(self):
        ...
        if edge is None or QgsWkbTypes.geometryType(edge.wkbType()) != QgsWkbTypes.LineGeometry:
            QtWidgets.QMessageBox.warning(self, "超距增点", "项目配置中没有有效的 Pole Edge 图层。")
            return
        self.accept()
...
class OverlengthPoleProcessor:
    ...
    def _meters_per_unit(self, p):
        ...
        try:
            vals = [
                self.da.measureLine(p, QgsPointXY(p.x() + 1, p.y())),
                self.da.measureLine(p, QgsPointXY(p.x(), p.y() + 1)),
            ]
    ...
    def _query_poles(self, line_geom):
        ...
        for iid in self._index.intersects(bb):
            ...
            pt = data[2]
            try:
                if line_geom.closestSegmentWithContext(pt)[0] <= r * r:
                    out.append((iid, data, pt))
            except Exception:
                pass
        return out
    ...
    def _choose_existing(self, pts, candidates):
        ...
        while True:
            ...
            _, d, p, role = best
            selected.append((d, p, role)); cuts.append(d)
            remaining = [x for x in remaining if abs(x[0] - d) >= 0.01]
            if self._gap_allowed(sorted(cuts), [self._type_at_distance(selected, x, total) for x in sorted(cuts)]) <= max(self.limits.values()):
                # The final dynamic pass below performs the exact pairwise check.
                pass
    ...
    @staticmethod
    def _gap_allowed(cuts, roles):
        if len(cuts) < 2: return 0.0
        return max(b - a for a, b in zip(cuts[:-1], cuts[1:]))

    @staticmethod
    def _type_at_distance(selected, d, total):
        for sd, _, role in selected:
            if abs(sd - d) < 0.01: return role
        return "New Pole"

    def _make_new_cuts(self, cut_specs, total, start_role, end_role):
        """Walk each gap and insert New Pole points using pairwise limits."""
        specs = sorted(cut_specs, key=lambda x: x[0])
        result = []
        boundaries = [(0.0, QgsPointXY(self._point_at_distance(specs, 0.0, total)), start_role)]
        for d, p, role in specs:
            boundaries.append((d, QgsPointXY(p), role))
        boundaries.append((total, QgsPointXY(self._point_at_distance(specs, total, total)), end_role))

        # Replace geometry-derived placeholder endpoints with exact original positions.
        boundaries[0] = (0.0, self._last_pts[0], start_role)
        boundaries[-1] = (total, self._last_pts[-1], end_role)

        for a, b in zip(boundaries[:-1], boundaries[1:]):
            left_d, left_p, left_role = a; right_d, right_p, right_role = b
            remaining = right_d - left_d
            cursor_d = left_d; cursor_p = left_p; cursor_role = left_role
            while remaining > self._allowed(cursor_role, right_role) + 1e-8:
                step = self._allowed(cursor_role, "New Pole")
                if step <= 0: break
                nd = cursor_d + step
                if nd >= right_d - 1e-8: break
                np, _ = self._point_at_distance(self._last_pts, nd)
                result.append((nd, np, "New Pole"))
                self._add_temp_point(self._temp_layer, np)
                self._stats["new"] += 1
                cursor_d, cursor_p, cursor_role = nd, np, "New Pole"
                remaining = right_d - cursor_d
        return sorted(specs + result, key=lambda x: x[0])

    def _point_at_distance(self, pts, distance, total=None):
        cum, total2 = self._cum_lengths(pts); total = total2 if total is None else total
    ...
    def _subline(self, pts, cut_specs):
        cum, total = self._cum_lengths(pts)
        cuts = [(0.0, QgsPointXY(pts[0]), "Endpoint")] + sorted(cut_specs, key=lambda x: x[0]) + [(total, QgsPointXY(pts[-1]), "Endpoint")]
        out = []
        for (da, pa, _), (db, pb, _) in zip(cuts[:-1], cuts[1:]):
    ...
    def _remove_duplicates(self, features):
        ...
        for feat in features:
            ...
            if g.isMultipart():
                ...
                keys.append(self._duplicate_key([QgsPointXY(p) for p in g.asPolyline()]))
            key = tuple(keys)
    ...
    def run(self):
        ...
        if layer is None:
            self._msg("项目配置中的 Pole Edge 不存在。", 2); return
        self._prepare_poles()
        ...
        for feat in unique:
            ...
            for pts in parts:
                ...
                self._last_pts = [QgsPointXY(p) for p in pts]
                _, total = self._cum_lengths(pts)
                candidates = self._query_poles(QgsGeometry.fromPolylineXY(self._last_pts))
                existing = self._choose_existing(pts, candidates)
                start_role = self._nearest_endpoint_role(pts[0], candidates) or "Existing Pole"
                end_role = self._nearest_endpoint_role(pts[-1], candidates) or "Existing Pole"
                cut_specs = [(d, QgsPointXY(p), role) for d, p, role in existing]
                dynamic = self._make_new_cuts(cut_specs, total, start_role, end_role)
                over = self._has_overlength(dynamic, total, start_role, end_role)
                if not over:
                    new_geoms.append(QgsGeometry.fromPolylineXY(self._last_pts)); continue
                self._stats["over"] += 1; changed = True
                dynamic = self._make_new_cuts(cut_specs, total, start_role, end_role)
                new_geoms.extend(self._subline(pts, dynamic)); self._stats["segments"] += len(dynamic) + 1
        ...
        self._msg(
            f"超距增点完成：检查超距线 {self._stats['over']} 条，新增 New Pole {self._stats['new']} 个，"
            f"删除重复线 {self._stats['duplicates']} 条。"
        )

    def _nearest_endpoint_role(self, point, candidates):
        best = None
        for _, data, _ in candidates:
            try:
                d = self.da.measureLine(QgsPointXY(point), data[2])
            except Exception:
                continue
    ...
    def _has_overlength(self, specs, total, start_role, end_role):
        ...
        for a, b in zip(ordered[:-1], ordered[1:]):
            if b[0] - a[0] > self._allowed(a[1], b[1]) + 1e-8: return True
        return False
