...
Keeps v4's project-scoped data/state logic and adds:
- compact main UI focused on the current FDT
- current-link total distance
- completed-design browser with FAT count and total distance
- FAT map overlays for completed FATs and current-link FATs
- hover tooltip with prospective segment/total distance
"""
...
def _current_fdt_fat_ids(dialog):
    result = set()
    for _, design in _current_fdt_designs(dialog):
...
class LinkDesignDialog(_V4LinkDesignDialog):
    """v5 Link Design UI while preserving v4 state/reconciliation behavior."""
    ...
    def _activate_map_tool(self):
        ...
        self._tool = LinkDesignMapToolV5(self.iface, self._engine, self)
    ...
    def _draw_fat_point(self, info, color, size=10):
        point = self._fat_point_canvas(info)
        ...
        band.setToGeometry(QgsGeometry.fromPointXY(point), self.iface.mapCanvas().mapSettings().destinationCrs())
        return band
    ...
    def _refresh_fat_overlays(self):
        ...
        if self._engine is None:
            return
        completed = _current_fdt_fat_ids(self)
    ...
    def _refresh_ui(self):
        # v4 owns all data/state behavior; only replace presentation.
        ...
        current_designs = _current_fdt_designs(self)
        self.link_count_label.setText(f"当前 FDT 已规划链路：{len(current_designs)}")
        self.planned_fat_label.setText(f"当前 FDT 已规划 FAT：{len(_current_fdt_fat_ids(self))}")
        ...
        self.distance_label.setText(
            "各段距离：" + ("、".join(f"{r['distance']:.1f}m" for r in routes) if routes else "—")
        )
    ...
    def open_completed_designs(self):
        ...
        dlg = CompletedDesignDialogV5(self)
        dlg.exec_()
        self._refresh_ui()
        self._refresh_fat_overlays()
...
class CompletedDesignDialogV5(_BaseCompletedDesignDialog):
    """Completed-design browser focused on FDT, Link, FAT count and distance."""
    ...
    def _build_ui(self):
        ...
        self.info = QtWidgets.QLabel("点击 Link 可在地图上高亮路线；双击 Link 可进入修改。")
        ...
        row.addWidget(self.modify_btn); row.addWidget(self.delete_btn); row.addWidget(self.write_btn); row.addWidget(close_btn)
        root.addLayout(row)
    ...
    def _refresh_tree(self):
        ...
        for fdt in sorted(grouped):
            ...
            fdt_distance = 0.0
            for _, d in entries:
                fdt_fats.update(int(x[0]) for x in d.get("nodes", []) if x)
                fdt_distance += float(d.get("length", 0.0) or 0.0)
                total_distance += float(d.get("length", 0.0) or 0.0)
            all_fats.update(fdt_fats)
            root = QtWidgets.QTreeWidgetItem([fdt, str(len(fdt_fats)), f"{fdt_distance:.1f}m", "已完成"])
            root.setData(0, Qt.UserRole, ("fdt", fdt))
            self.tree.addTopLevelItem(root)
            root.setExpanded(True)
            for index, design in sorted(entries, key=lambda x: str(x[1].get("link", ""))):
                fat_count = self._fat_count(design)
                distance = float(design.get("length", 0.0) or 0.0)
                child = QtWidgets.QTreeWidgetItem([
                    str(design.get("link", "L?")),
                    str(fat_count),
                    f"{distance:.1f}m",
                    "已完成",
                ])
    ...
    def _on_clicked(self, item, column):
        ...
        if target[0] == "fdt":
            ...
            fats = set()
            for _, d in entries:
                fats.update(int(x[0]) for x in d.get("nodes", []) if x)
            self.info.setText(f"{fdt}：{len(entries)} 条 Link，{len(fats)} 个 FAT，总路径 {total:.1f}m。")
        else:
            index = int(target[1])
            if 0 <= index < len(self.main_dialog._designs):
                ...
                self.info.setText(
                    ...
                    f"{self._fat_count(design)} 个 FAT，{float(design.get('length', 0.0)):.1f}m，已完成。"
                )
    ...
    def _delete_selected(self):
        ...
        if self.main_dialog.delete_link(index):
            self._refresh_tree()
            self.main_dialog._refresh_fat_overlays()
...
class LinkDesignMapToolV5(_BaseMapTool):
    """Map tool with completed/current FAT visual states and hover distance."""

    def _fat_state(self, info):
        fid = int(info.get("feature_id"))
        current = _current_link_fat_ids(self.dialog)
        completed = _current_fdt_fat_ids(self.dialog)
        if fid in current:
            return "current"
        if fid in completed:
            return "completed"
    ...
    def canvasMoveEvent(self, event):
        if not self.dialog._draw_active or not self.dialog._sequence:
            self._clear_hover()
            self.canvas.unsetMapTool(self) if False else None
            return
        ...
        if not hit:
            ...
            return
        _, _, info = hit
        state = self._fat_state(info) if info["typ"] == "FAT" else "available"
        route = self.dialog.prospective_route(info)
        if route:
            self._draw_hover(route)
            committed = []
            if self.dialog._engine:
                for first, second in zip(self.dialog._sequence[:-1], self.dialog._sequence[1:]):
                    existing = self.dialog._engine.route(first[0], first[1], second[0], second[1])
                    if existing:
                        committed.append(float(existing["distance"]))
            current_distance = float(route.get("distance", 0.0))
            total_distance = sum(committed) + current_distance
            label = info.get("label", "")
            if info["typ"] == "FAT":
                if state == "completed":
                    text = f"{label}\n已完成设计 FAT\n当前段：{current_distance:.1f} m\n链路总距离：{total_distance:.1f} m"
                elif state == "current":
                    text = f"{label}\n当前 Link 已选择\n当前段：{current_distance:.1f} m\n链路总距离：{total_distance:.1f} m"
                else:
                    text = f"{label}\n当前段：{current_distance:.1f} m\n链路总距离：{total_distance:.1f} m"
            else:
                text = f"{label}\n预计当前段：{current_distance:.1f} m\n预计链路总距离：{total_distance:.1f} m"
            try:
                QtWidgets.QToolTip.showText(event.globalPos(), text, self.canvas)
            except Exception:
                pass
